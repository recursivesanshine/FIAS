...
def visualize_distribution(df, category):
    ...
    sns.violinplot(x="percentage", y="subcategory", data=df[df['main_category'] == category], palette="vlag", inner="point")
    plt.title(f'Distribution of Subcategories within {category}')
    plt.show()

def create_plot(df):
    plt.figure(figsize=(12, 8))
    sns.violinplot(x="percentage", y="main_category", data=df, palette="vlag", inner="point")
...
def save_and_log_plot(df, log_file):
    output_file = 'cumulated_results_violinplot.png'
...
def main():
    parser = argparse.ArgumentParser(description='Create violin plots from a CSV file or a folder containing CSV files')
